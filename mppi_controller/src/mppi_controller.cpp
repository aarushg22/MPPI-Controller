...
Twist CustomMPPIController::computeVelocityCommands(
    ...
    std::vector<std::vector<Twist>> noisy_controls(params_.batch_size, 
        std::vector<Twist>(params_.time_steps));
    
    std::vector<std::vector<Pose>> state_trajectories(params_.batch_size, 
        std::vector<Pose>(params_.time_steps + 1));
    ...
    computeTrajectoryCosts(trajectory_costs, state_trajectories, noisy_controls, current_path_, *costmap_info);
    
    // Update optimal control sequence
    updateControlSequence(optimal_control_sequence_, noisy_controls, trajectory_costs);
...
void CustomMPPIController::generateNoisyTrajectories(
    ...
    for (int i = 0; i < params_.batch_size; ++i) {
        for (int j = 0; j < params_.time_steps; ++j) {
            ...
            // Apply constraints
            trajectories[i][j].linear_x = std::clamp(trajectories[i][j].linear_x, params_.vx_min, params_.vx_max);
            trajectories[i][j].linear_y = std::clamp(trajectories[i][j].linear_y, params_.vy_min, params_.vy_max);
            trajectories[i][j].angular_z = std::clamp(trajectories[i][j].angular_z, params_.wz_min, params_.wz_max);
            ...
            if (j > 0) {
                ...
                trajectories[i][j].linear_x = std::clamp(
                    trajectories[i][j].linear_x,
                    prev.linear_x - max_vx_change,
                    prev.linear_x + max_vx_change
                );
                
                trajectories[i][j].linear_y = std::clamp(
                    trajectories[i][j].linear_y,
                    prev.linear_y - max_vy_change,
                    prev.linear_y + max_vy_change
                );
                
                trajectories[i][j].angular_z = std::clamp(
                    trajectories[i][j].angular_z,
...
void CustomMPPIController::propagateTrajectories(
    ...
    for (int i = 0; i < params_.batch_size; ++i) {
        ...
        for (int j = 0; j < params_.time_steps; ++j) {
            state_trajectories[i][j+1] = predictMotionModel(
                state_trajectories[i][j],
                control_trajectories[i][j],
                params_.model_dt
            );
        }
    }
...
Pose CustomMPPIController::predictMotionModel(
    const Pose& start,
    const Twist& control,
    double dt
) {
    ...
    next.position.x = start.position.x + control.linear_x * cos(yaw) * dt - control.linear_y * sin(yaw) * dt;
    next.position.y = start.position.y + control.linear_x * sin(yaw) * dt + control.linear_y * cos(yaw) * dt;
    
    // Update orientation
    next.orientation = start.orientation + control.angular_z * dt;
    
    // Normalize orientation to [-π, π]
    while (next.orientation > M_PI) next.orientation -= 2.0 * M_PI;
    while (next.orientation < -M_PI) next.orientation += 2.0 * M_PI;
...
void CustomMPPIController::computeTrajectoryCosts(
    ...
    for (int i = 0; i < params_.batch_size; ++i) {
        ...
        double obstacle_cost = computeObstacleCost(state_trajectories[i], costmap_info);
...
double CustomMPPIController::computePathCost(
...
double CustomMPPIController::computeObstacleCost(
    const std::vector<Pose>& trajectory,
    const CostmapInfo& costmap_info
) {
    double total_cost = 0.0;
    ...
    for (const auto& pose : trajectory) {
        ...
        if (idx >= 0) {
            ...
            if (row >= 0 && row < costmap_info.height && col >= 0 && col < costmap_info.width) {
...
double CustomMPPIController::computeGoalCost(
    const std::vector<Pose>& trajectory,
    const Path& reference_path
) {
    if (reference_path.poses.empty()) {
...
void CustomMPPIController::updateControlSequence(
    ...
    for (int j = 0; j < params_.time_steps; ++j) {
        ...
        control_sequence[j].linear_x = params_.gamma * control_sequence[j].linear_x + 
                                       (1.0 - params_.gamma) * weighted_control.linear_x;
        control_sequence[j].linear_y = params_.gamma * control_sequence[j].linear_y + 
                                       (1.0 - params_.gamma) * weighted_control.linear_y;
        control_sequence[j].angular_z = params_.gamma * control_sequence[j].angular_z + 
                                        (1.0 - params_.gamma) * weighted_control.angular_z;
        
        // Apply constraints
        control_sequence[j].linear_x = std::clamp(control_sequence[j].linear_x, params_.vx_min, params_.vx_max);
        control_sequence[j].linear_y = std::clamp(control_sequence[j].linear_y, params_.vy_min, params_.vy_max);
        control_sequence[j].angular_z = std::clamp(control_sequence[j].angular_z, params_.wz_min, params_.wz_max);
    }
}

int CustomMPPIController::getCostmapIndex(
    const Point& point,
    const CostmapInfo& costmap_info
) {
    // Convert world coordinates to costmap coordinates
    int mx = static_cast<int>((point.x - costmap_info.origin.x) / costmap_info.resolution);
    int my = static_cast<int>((point.y - costmap_info.origin.y) / costmap_info.resolution);
    
    // Check if point is inside costmap bounds
    if (mx < 0 || mx >= costmap_info.width || my < 0 || my >= costmap_info.height) {
        return -1;
    }
